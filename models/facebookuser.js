...
module.exports = function(sequelize, DataTypes) {
    return sequelize.define("facebookusers", {
	facebookid: {type: DataTypes.STRING(20), unique: true, allowNull:false},
    }, {
	classMethods: {
	    numPersonaUsers: function() {
		this.count().success(function(c) {
		    console.log("There are %s unique Facebook Users", c);});
	    },
		...
		*/
	    ...
	    addFacebookUserElseContinue: function(IDFacebook, cb) {
		/*
		...
		*/
		var userFacebookID  = IDFacebook;
		var _User = this;
		_User.find({where: {facebookid: userFacebookID}}).success(function(dbFacebookID) {
			if (dbFacebookID) {
			    // fb user already exists, do nothing
			    console.log("fb user exists! Welcome back :)");
			    cb();
			} else {
			    console.log("fb user doesn't exist - Woop! creating...");
				     if(userFacebookID.length > 19){
					 console.log("Oops crazy long facebook ids not welcome here!");
					 cb();
				     }
