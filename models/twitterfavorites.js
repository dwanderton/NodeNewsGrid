...
module.exports = function(sequelize, DataTypes) {
    return sequelize.define("twitterfavorites", {
	twitterid: {type: DataTypes.STRING(20), allowNull:false},
	bbcpublished: {type: DataTypes.BIGINT, allowNull: false}
    }, {
	classMethods: {
	    storiesFavoritedSince: function(dateSince, successcb, errcb){
	    ...
	    listOfStoriesFavorited: function(userID, successcb, errcb) {
                this.findAll({where: {twitterid: userID}, attributes: ['bbcpublished']}).success(function(storiesRead) {
                    successcb(storiesRead);
                }).error(errcb);
            },
		...
		*/
	    ...
	    addStoryFavoritedElseContinue: function(twitterID, bbcPublishedID, cb) {
		/*
		...
		*/
		...
		var _User = this;
		_User.find({where: {twitterid: userID, bbcpublished: storyID}}).success(function(dbUserEmail) {
			if (dbUserEmail) {
			    // Story already exists as read, do nothing
			    console.log("%s has already Favourited story: %s", userID, storyID);
			    cb();
			} else {
