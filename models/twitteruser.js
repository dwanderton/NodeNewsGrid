...
module.exports = function(sequelize, DataTypes) {
    return sequelize.define("twitterusers", {
	twitterid: {type: DataTypes.STRING(20), unique: true, allowNull:false},
    }, {
	classMethods: {
	    numPersonaUsers: function() {
		this.count().success(function(c) {
		    console.log("There are %s unique Twitter Users", c);});
	    },
		...
		*/
	    ...
	    addTwitterUserElseContinue: function(IDtwitter, cb) {
		/*
		...
		*/
		...
		var userTwitterID  = IDtwitter;
		var _User = this;
		_User.find({where: {twitterid: userTwitterID}}).success(function(dbTwitterID) {
			if (dbTwitterID) {
			    // order already exists, do nothing
			    console.log("Twitter user exists! Welcome back :)");
			    cb();
			} else {
			    console.log("Twitter user doesn't exist - Woop! creating...");
				     if(userTwitterID.length > 19){
					 console.log("Oops crazy long twitter id not welcome here!");
					 cb();
				     }
